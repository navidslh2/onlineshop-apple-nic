...
const MenuBag = ({ isShowMenuBag, showMenuBagHandler }: Props) => {
  return (
    <AnimatePresence>
      {isShowMenuBag && (
        <motion.div className="absolute flex flex-col gap-8 bg-white p-5 rounded-2xl top-15  max-sm:left-0 max-sm:right-0 sm:top-11 sm:-left-2 sm:w-[300px] after:absolute after:content[''] after:bottom-full after:left-16 sm:after:left-3 after:border-8 after:border-transparent after:border-b-white origin-top z-50"
        initial={{scaleY:0}}
        animate={{scaleY:1}}
        exit={{scaleY:0}}
        ...
        >
          ...
          <ul className="flex flex-col gap-5">
            ...
            <li className="flex items-center justify-start gap-2 font-thin text-sm text-blue-900">
              ...
              <Link href="/Account/login" className="hover:underline" onClick={showMenuBagHandler} >
                ورود
              </Link>
            </li>
          </ul>
